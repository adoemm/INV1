...
<%    try {
        if (fine) {
            if (request.getParameter(WebUtil.encode(session, "imix")) != null) {
                ...
                access4ThisPage.add("ConfirmarBaja");
        ...
        <div id="wrapper">
            <div id="divBody">
                ...
                <div class="form-container" width="100%">                    
                    ...
                    <form name="updateBaja" method="post" action="" enctype="application/x-www-form-urlencoded" id="updateBaja">
                        <input type="hidden" name="<%=WebUtil.encode(session, "imix")%>" value="<%=WebUtil.encode(session, UTime.getTimeMilis())%>"/>
                        <input type="hidden" name="FormFrom" value="updateBaja"/>
                        <input type="hidden" name="idSolicitud" value="<%=request.getParameter("idSolicitud")%>"/>
                        <fieldset>
                            <legend>Confirmar Solicitud</legend>
                            <div>
                                <label for="noOficio">*No. de confirmación del DGSEI</label>
                                <input type="text" name="noOficio">
                            </div>
                            <div>
                                <label for="fechaBaja">*Fecha de Baja</label>
                                <input type="text" class="w8em format-y-m-d divider-dash highlight-days-67" id="fechaBaja"   name="fechaBaja" value="<%=UTime.calendar2SQLDateFormat(Calendar.getInstance())%>" size="10" MAXLENGTH="10">
                            </div>
                            <div id="botonEnviarDiv" >
                                <input type="button" value="Guardar" name="Enviar" onclick="enviarInfo(document.getElementById('updateBaja'));"/>
                            </div>      
                        </fieldset>
                    </form>
                    <div id="divResult"> 
