...
<%    try {
        if (request.getParameter(WebUtil.encode(session, "imix")) != null) {
            ...
            if (UserUtil.isAValidUser(access4ThisPage, userAccess)) {
                if(!WebUtil.decode(session, request.getParameter("idModelo")).equalsIgnoreCase("Todos")){
                LinkedList modelo = QUID.select_Modelo(WebUtil.decode(session, request.getParameter("idModelo")));
%>
<%=modelo.get(4).toString().trim().equalsIgnoreCase("")?"Sin información":modelo.get(4)%>
<%            }
                } else {
