    ...
    try {
        if (fine) {
            if (request.getParameter(WebUtil.encode(session, "imix")) != null) {
                ...
                access4ThisPage.add("ConsultaTipoDocumento");
                ...
                "maxDots": 6
                    ...
                    <div id="contenent_info" style="display:none;">
                        <form name="usuario" method="post" action="" enctype="application/x-www-form-urlencoded" id="form">
                            <fieldset>
                                <legend>Catálogo de Documentos</legend>
                                ...
                                <table border="0" width="100%" cellpadding="0" cellspacing="0" class="display" id="example">
                                    ...
                                    <tbody>
                                        <%
                                            int i = 1;
                                            it = QUID.select_Tipo_Archivo().iterator();
                                            while (it.hasNext()) {
                                        ...
                                        <tr id="<%=WebUtil.encode(session, listAux.get(0))%>" class="gradez">       
                                            ...
                                            <td id="rOnly" class="rOnly"><a href="<%=PageParameters.getParameter("mainController")%>?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&idDocumento=<%=WebUtil.encode(session, listAux.get(0))%>&FormFrom=deleteTipoArchivo"><img src="<%=PageParameters.getParameter("imgRsc")%>/icons/Gnome-Process-Stop-64.png" title="Eliminar" width="22" height="23" alt="Eliminar"></a></td>
                                        </tr>
                                        <%
                                                i += 1;
                                            }
                    ...
                    <div>
                        <form action="<%=PageParameters.getParameter("mainController")%>" method="POST">                            
                            <input type="hidden" name="FormFrom" id="FormFrom" value="insertDocumentos">
                            ...
                            <fieldset>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="nombreDocumento" size="45">
                                </div>
