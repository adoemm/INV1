...
<%    try {
        if (request.getParameter(WebUtil.encode(session, "imix")) != null) {
            ...
            if (UserUtil.isAValidUser(access4ThisPage, userAccess)) {
                Iterator it = null;
                it = QUID.select_Departamento_PlantelXID(WebUtil.decode(session, request.getParameter("idPlantel"))).iterator();
                String onChange = "";
                if (request.getParameter("onChange") != null) {
                    switch (request.getParameter("onChange")) {
                        case "getGrupos":
                            onChange = "getGrupos(this.value)";
                            break;
                    }
                }
...
<table width="95%" border="0">
    ...
    <tr>
        ...
        %>
        <td>
            <input type="checkbox" name="idDepartamento" value="<%=WebUtil.encode(session, datos.get(0))%>" style="padding: 0px; margin:0px;cursor: pointer;">
            <%
            if(WebUtil.decode(session, request.getParameter("idPlantel")).equalsIgnoreCase("todos")){
                %>
                <%=datos.get(4).toString().toUpperCase()%>/ 
                <%
            }
            %>
            <%=datos.get(3)%>
