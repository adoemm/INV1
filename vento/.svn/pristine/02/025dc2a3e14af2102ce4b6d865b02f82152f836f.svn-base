...
<html lang="<%=PageParameters.getParameter("Content-Language")%>">
    <head>
        <title>Consulta por Patrones</title>
        ...
        <script type="text/javascript" language="javascript" charset="utf-8">
            ...
            function enviarInfo(form, action) {
                if (action === "exportar") {
                    form.action = '<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("excelReports")%>/ConsultaXPatron.jsp?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>';
                            }
                            form.submit();
                        }
            ...
            <div id="divBody">
                ...
                <div class="form-container" width="100%">                    
                    ...
                    <form id="report" name="report" method="post" enctype="application/x-www-form-urlencoded" target="_blank" action="<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("prints")%>/ResumenGeneral.jsp">
                        ...
                        <fieldset>
                            <legend>Consulta por Patrones</legend>
                            <div>
                            ...
                            <div>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="patron" size="150">
                                </div>
                                <div>
                                    <label>Ejemplo de Patron:</label>
                                    <input type="text" name="ejemploPatron" value="[M][X][J][7, 8][2, 5][2][0][3][1, 2, 3, 4, 5][0-9a-z]" size="150">
                                </div>
                                <div>
                                    <input type="radio" name="match" value="igual" checked>Los No. de serie coinciden con el patron
                                    <input type="radio" name="match" value="diferente">Los No. de serie difieren con el patron
                                    <input type="radio" name="match" value="sm">Buscar en registros sin marca, sin modelo
                                </div>
