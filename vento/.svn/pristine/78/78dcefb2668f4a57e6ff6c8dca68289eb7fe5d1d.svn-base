    ...
    try {
        if (fine) {
            if (request.getParameter(WebUtil.encode(session, "imix")) != null) {
                ...
                access4ThisPage.add("ConsultaActividad");
...
<html lang="<%=PageParameters.getParameter("Content-Language")%>">
        ...
        <script type="text/javascript" language="javascript" charset="utf-8">
            ...
            function popupArchivos(idObjeto) {
                newwindow = window.open(
                        '<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("gui")%>/Insert_ObjetoArchivo.jsp?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&idObjeto='+idObjeto+'&nombreObjeto=<%=WebUtil.encode(session, "ACTIVIDAD_PLANTEL")%>'
                                        , 'Archivos'
    ...
    <body  onload="populate();">
        ...
        function populate()
        {
            ...
            cells[8] = '<img src="<%=PageParameters.getParameter("imgRsc")%>/icons/attachment.png" onclick="popupArchivos(\'<%=WebUtil.encode(session, listAux.get(21))%>\');" title="Ver Archivos" width="22" height="23" alt="Ver Archivos">';
