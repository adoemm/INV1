...
<%    try {
        if (fine) {
            if (request.getParameter(WebUtil.encode(session, "imix")) != null) {
                ...
                access4ThisPage.add("SinNoInventario");
...
<html lang="<%=PageParameters.getParameter("Content-Language")%>">
    <head>
        <title>Bienes Sin No. Inventario</title>
        ...
        <script type="text/javascript" language="javascript" charset="utf-8">
            ...
            function enviarInfo(form, action) {
                if (action === "imprimir") {
                    form.action = '<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("prints")%>/SinNoInventario.jsp';
                } else if (action === "exportar") {
                    form.action = '<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("excelReports")%>/SinNoInventario.jsp?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>';
                            }
                            form.submit();
                        }
            ...
            <div id="divBody">
                ...
                <div class="form-container" width="100%">                    
                    ...
                    <form id="report" name="report" method="post" enctype="application/x-www-form-urlencoded" target="_blank" action="<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("prints")%>/ResumenGeneral.jsp">
                        ...
                        <fieldset>
                            <legend>Bienes Sin No. Inventario</legend>
                            <div>
