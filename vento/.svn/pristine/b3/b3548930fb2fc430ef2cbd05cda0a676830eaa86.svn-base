    ...
    <head>
        ...
        <script type="text/javascript" language="javascript" charset="utf-8">
            ...
            function enviarInfo(form, action) {
                if (action === "imprimir") {
                    form.action = '<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("prints")%>/ResumenBien.jsp';
                } else if (action === "exportar") {
                    form.action = '<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("excelReports")%>/ResumenBien.jsp?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>';
                            }
                            form.submit();
                        }
        ...
        <div id="wrapper">
            <div id="divBody">
                ...
                <div class="form-container" width="100%">                    
                    ...
                    <form id="report" name="report" method="post" enctype="application/x-www-form-urlencoded" target="_blank" action="<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("prints")%>/ResumenBien.jsp">
                        <input type="hidden" name="<%=WebUtil.encode(session, "imix")%>" value="<%=WebUtil.encode(session, UTime.getTimeMilis())%>"/>
                        <fieldset>
                            <legend>Resumen de Bienes</legend>
                            <div>
                        ...
                        <input type="button" value="Imprimir" name="Imprimir" onclick=" enviarInfo(document.getElementById('report'),'imprimir');"/>
