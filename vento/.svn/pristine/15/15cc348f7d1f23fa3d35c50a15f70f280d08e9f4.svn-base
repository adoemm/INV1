    ...
    try {
        if (fine) {
            if (request.getParameter(WebUtil.encode(session, "imix")) != null) {
                ...
                access4ThisPage.add("ConsultaIncidente");
...
<html lang="<%=PageParameters.getParameter("Content-Language")%>">
    <head>
        <title>Catálogo de Incidentes</title>
    ...
    <body  onload="populate();">
                ...
                <div class="form-container" width="100%">
                    ...
                    <div id="contenent_info" >
                        <form name="usuario" method="post" action="" enctype="application/x-www-form-urlencoded" id="form">
                            <fieldset>
                                <legend>Catálogo de Incidentes</legend>
                    ...
                    <a href="<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("gui")%>/InsertIncidente.jsp?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>">
        ...
        function populate()
        {
            ...
            var cells = new Array();
                    ...
                    <%
                        int cont = 0;
                        it = QUID.select_Incidente().iterator();
                        while (it.hasNext()) {
            ...
            cells[2] = '<a href="<%=PageParameters.getParameter("mainController")%>?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&idIncidente=<%=WebUtil.encode(session, listAux.get(0))%>&FormFrom=deleteIncidente"><img src="<%=PageParameters.getParameter("imgRsc")%>/icons/Gnome-Process-Stop-64.png" title="Eliminar" width="22" height="23" alt="Eliminar"></a>';
