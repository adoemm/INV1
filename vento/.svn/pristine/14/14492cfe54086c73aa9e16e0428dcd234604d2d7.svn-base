    ...
    try {
        if (fine) {
            if (request.getParameter(WebUtil.encode(session, "imix")) != null) {
                ...
                access4ThisPage.add("ConsultaTipoCompra");
            ...
            function habilitarEditarColumna() {
                ...
                oTable.$("td[id!='rOnly']").editable('<%=PageParameters.getParameter("mainController")%>', {
                    ...
                    , "submitdata": function(value, settings) {
                        $("#res").val("ID_Tipo_Compra_" + this.parentNode.getAttribute('id'));
                        return {
                            _method: "post"
                            , "row_id": this.parentNode.getAttribute('id')
                            , "column": oTable.fnGetPosition(this)[1]
                            , "FormFrom": 'updateTipo_Compra'
        ...
        <div id="wrapper">
            <div id="divBody">
                ...
                <div class="form-container" width="100%">
                                ...
                                <table border="0" width="100%" cellpadding="0" cellspacing="0" class="display" id="example">
                                    ...
                                    <tbody>
                                        ...
                                        <tr id="<%=WebUtil.encode(session, listAux.get(0))%>" class="gradez">       
                                            <td id="rOnly"><%=i%></td>                                        
                                            <td><%=listAux.get(1)%></td>
                                            <td id="rOnly" class="rOnly"><a href="<%=PageParameters.getParameter("mainController")%>?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&ID_Tipo_Compra=<%=WebUtil.encode(session, listAux.get(0))%>&FormFrom=deleteTipo_Compra"><img src="<%=PageParameters.getParameter("imgRsc")%>/icons/Gnome-Process-Stop-64.png" title="Eliminar tipo de compra" width="22" height="23" alt="Eliminar tipo de compra"></a></td>
                                        </tr>
                                        <%
                                            }
                                        %>  
                    ...
                    <a href="<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("gui")%>/InsertTipo_Compra.jsp?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>">
                        <button>Nuevo Tipo de Compra</button>
                    </a> 
                </div>                
                <div id="divFoot">
