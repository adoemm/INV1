    ...
    <head>
        ...
        <script type="text/javascript" language="javascript" charset="utf-8">
            ...
            function enviarInfo(form, action) {
                if (action === "exportar") {
                    form.action = '<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("excelReports")%>/MovimientosSalida.jsp';
                    form.submit();
                } else
                {
                    form.action = '<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("prints")%>/MovimientoSalidaTotal.jsp';
                    form.submit();
                }
            }
        ...
        <div id="wrapper">
            <div id="divBody">
                ...
                <div class="form-container" width="100%">                    
                    ...
                    <table width="100%" height="25px" border="0" align="center" cellpadding="0" cellspacing="0">
                        <tr>
                            <td width="64%" height="25" align="left" valign="top">
                                ...
                                > Reportes de Movimientos de Salida
                            </td>
                    ...
                    <form id="report" name="report" method="post" enctype="application/x-www-form-urlencoded" target="_blank" action="<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("prints")%>/MovimientoSalidaTotal.jsp?plantelActual">
                        <input type="hidden" name="<%=WebUtil.encode(session, "imix")%>" value="<%=WebUtil.encode(session, UTime.getTimeMilis())%>"/>
                        <fieldset>
                            <legend>Reportes Movimientos de Salida</legend>
                            <div>
                                <jsp:include page='<%=PageParameters.getParameter("ajaxFunctions") + "/getSelectPlantelSection.jsp?conTokyo=1"%>' flush = 'true'>
                        ...
                        <input style="margin-left: 240px" type="button" value="Imprimir" name="Imprimir" onclick=" enviarInfo(document.getElementById('report'), 'imprimir')"/>
                        <input style="margin-left: 0px" type="button" value="Exportar a excel" name="Exportar" onclick=" enviarInfo(document.getElementById('report'), 'exportar')"/>
                    </form>
                </div>   
                <div id="divFoot">
