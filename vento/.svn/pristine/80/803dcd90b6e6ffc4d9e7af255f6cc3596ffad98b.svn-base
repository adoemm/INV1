...
<label for="FK_ID_Personal_Recibe">*Responsable de Recepción:</label>
<select name="FK_ID_Personal_Recibe" >                                            
    <option value=""></option>
    <%
        it = null;
       it = QUID.select_NombreEncargadoPlantel(WebUtil.decode(session, request.getParameter("FK_ID_Plantel"))).iterator();
        while (it.hasNext()) {
            listAux = null;
