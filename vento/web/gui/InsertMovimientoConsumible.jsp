...
<%    try {
        if (fine) {
            if (request.getParameter(WebUtil.encode(session, "imix")) != null) {
                ...
                access4ThisPage.add("InsertMovimientoConsumible");
                    ...
                    , success: function(result) {
                        if (result === "SI") {
                            $.ajax({type: 'POST'
                                ...
                                , async: false
                                , url: "<%=PageParameters.getParameter("mainController")%>"
                                , data: '<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&idMovimientoConsumible=' + idMovimientoConsumible + '&FormFrom=deleteConsumibleMovimiento&idMovimiento=' + idMovimiento+'&idPlantel=<%=request.getParameter("idPlantel")%>'
                                , success: function(response) {
                                    if (response.indexOf("title: \"Error\"") >= 0) {
                                        $('#wrapper').find('#divResult').html(response);
                                    } else {
        ...
        <div id="wrapper">
            <div id="divBody">
                ...
                <div class="form-container" width="100%">                    
                    ...
                    <form name="insertEntradaConsumible" method="post" action="" enctype="application/x-www-form-urlencoded" id="insertEntradaConsumible">
                        <input type="hidden" name="<%=WebUtil.encode(session, "imix")%>" value="<%=WebUtil.encode(session, UTime.getTimeMilis())%>"/>
                        <input type="hidden" name="FormFrom" value="insertEntradaConsumible"/>
            ...
            hideButton('divAyuda');" title="Abrir ayuda">
                        ...
                        <div id="botonEnviarDiv" >
                            <input type="button" value="Guardar" name="Enviar" onclick="enviarInfo(document.getElementById('insertEntradaConsumible'));"/>
                            <input type="button" value="Nuevo" name="Nuevo" onclick="resetForm();"/>
                            <a href="<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("prints")%>/movimientoEntrada.jsp?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&idMovimiento=<%=request.getParameter("idMovimiento")%>" target="_blank">
                                <input type="button" value="Imprimir" name="Imprimir" onclick=""/>
                            </a>
                            <a href="<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("gui")%>/ConsultaMovimientoEntrada.jsp?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&idPlantel=<%=request.getParameter("idPlantel")%>">
                                <input type="button" value="Terminar" name="Terminar" onclick=""/>
                            </a>
                        </div>      
                    </form>
                    <br><br>
