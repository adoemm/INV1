    ...
    try {
        if (fine) {
            if (request.getParameter(WebUtil.encode(session, "imix")) != null) {
                ...
                access4ThisPage.add("ConsultaGarantia");
    ...
    <body  onload="populate();">
                ...
                <div class="form-container" width="100%">
                    ...
                    <div id="contenent_info" >
                        <form name="usuario" method="post" action="" enctype="application/x-www-form-urlencoded" id="form">
                            <fieldset>
                                <legend>Garantía del Bien</legend>
                    ...
                    <a href="<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("gui")%>/InsertGarantia.jsp?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&idBien=<%=request.getParameter("idBien")%>&idPlantel=<%=request.getParameter("idPlantel")%>">
                        <button>Registrar Garantía</button>
                    </a> 
                </div>
                <div id="divFoot">
        ...
        function populate()
        {
            ...
            var cells = new Array();
                    ...
                    <%
                        int cont = 0;
                        it = QUID.select_Garantia4Bien(WebUtil.decode(session, request.getParameter("idBien"))).iterator();
                        while (it.hasNext()) {
                            listAux = (LinkedList) it.next();
                    %>
            ids[<%=cont%>] = '<%=WebUtil.encode(session, listAux.get(7))%>';
            cells[0] = '<%=listAux.get(5).toString().replaceAll("'", "&#39;")%>';
            ...
            cells[4] = '<%=listAux.get(6).toString().replaceAll("'", "&#39;")%>';
            cells[5] = '<%=listAux.get(8).toString().replaceAll("'", "&#39;")%>';
            cells[6] = '<%=listAux.get(9).toString().replaceAll("'", "&#39;")%>';
            cells[7] = '<a href="<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("gui")%>/UpdateGarantia.jsp?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&idGarantia=<%=WebUtil.encode(session, listAux.get(7))%>&idBien=<%=request.getParameter("idBien")%>&idPlantel=<%=request.getParameter("idPlantel")%>"><img src="<%=PageParameters.getParameter("imgRsc")%>/icons/Gnome-Accessories-Text-Editor-64.png" title="Editar" width="22" height="23" alt="Editar"></a>';
            cells[8] = '<a href="<%=PageParameters.getParameter("mainController")%>?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&idGarantia=<%=WebUtil.encode(session, listAux.get(7))%>&idBien=<%=request.getParameter("idBien")%>&FormFrom=deleteGarantia&idPlantel=<%=request.getParameter("idPlantel")%>"><img src="<%=PageParameters.getParameter("imgRsc")%>/icons/Gnome-Process-Stop-64.png" title="Eliminar" width="22" height="23" alt="Eliminar"></a>';
