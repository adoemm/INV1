    ...
    try {
        if (fine) {
            if (request.getParameter(WebUtil.encode(session, "imix")) != null) {
                ...
                access4ThisPage.add("ConsultaNombreSoftware");
...
<html lang="<%=PageParameters.getParameter("Content-Language")%>">
    <head>
        <title>Catálogo Denominacion de Software</title>
            ...
            function habilitarEditarColumna() {
                ...
                oTable.$("td[id!='rOnly']").editable('<%=PageParameters.getParameter("mainController")%>', {
                    ...
                    , "submitdata": function(value, settings) {
                        $("#res").val("ID_Nombre_Software_" + this.parentNode.getAttribute('id'));
                        return {
                            _method: "post"
                            , "row_id": this.parentNode.getAttribute('id')
                            , "column": oTable.fnGetPosition(this)[1]
                            , "FormFrom": 'updateNombreSoftware'
                    ...
                    <table width="100%" height="25px" border="0" align="center" cellpadding="0" cellspacing="0">
                        <tr>
                            <td width="64%" height="25" align="left" valign="top">
                                ...
                                > Catálogo de Denominaciones de Software
                            </td>
                ...
                "maxDots": 6
                    ...
                    <div id="contenent_info" style="display:none;">
                        <form name="usuario" method="post" action="" enctype="application/x-www-form-urlencoded" id="form">
                            <fieldset>
                                <legend>Catálogo de Nombre de Software</legend>
                                ...
                                <table border="0" width="100%" cellpadding="0" cellspacing="0" class="display" id="example">
                                    ...
                                    <tbody>
                                        ...
                                        <tr id="<%=WebUtil.encode(session, listAux.get(0))%>" class="gradez">       
                                            <td id="rOnly"><%=i%></td>                                        
                                            <td><%=listAux.get(1)%></td>
                                            <td id="rOnly" class="rOnly"><a href="<%=PageParameters.getParameter("mainController")%>?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&ID_Nombre_Software=<%=WebUtil.encode(session, listAux.get(0))%>&FormFrom=deleteNombreSoftware"><img src="<%=PageParameters.getParameter("imgRsc")%>/icons/Gnome-Process-Stop-64.png" title="Eliminar licencia" width="22" height="23" alt="Eliminar licencia"></a></td>
                                        </tr>
                                        <%
                                            }
                                        %>  
                    ...
                    <a href="<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("gui")%>/InsertNombreSoftware.jsp?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>">
