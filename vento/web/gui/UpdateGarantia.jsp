...
<%    try {
        if (fine) {
            if (request.getParameter(WebUtil.encode(session, "imix")) != null) {
                ...
                access4ThisPage.add("UpdateGarantia");
        ...
        <div id="wrapper">
            <div id="divBody">
                ...
                <div class="form-container" width="100%">                    
                    ...
                    <form name="updateGarantia" method="post" action="" enctype="application/x-www-form-urlencoded" id="updateGarantia">
                        <input type="hidden" name="<%=WebUtil.encode(session, "imix")%>" value="<%=WebUtil.encode(session, UTime.getTimeMilis())%>"/>
                        <input type="hidden" name="FormFrom" value="updateGarantia"/>
                        <input type="hidden" name="idBien" value="<%=request.getParameter("idBien")%>"/>
                        <input type="hidden" name="idGarantia" value="<%=request.getParameter("idGarantia")%>"/>
                        <input type="hidden" name="idPlantel" value="<%=request.getParameter("idPlantel")%>"/>
                        <fieldset>
                            <legend>Editar Garantía</legend>
                            <div>
                                <%
                                    LinkedList garantia = QUID.select_Garantia(WebUtil.decode(session, request.getParameter("idGarantia")));
                                ...
                                <select name="tipoGarantia">
                                    <optgroup label="Tipo actual">
                                        <option value="<%=WebUtil.encode(session, garantia.get(10))%>"><%=garantia.get(6)%></option>
                                    </optgroup>
                                    <%
                                        while (t.hasNext()) {
                                ...
                                <input type="hidden" name="tipoActual" value="<%=WebUtil.encode(session, garantia.get(10))%>"/>
                            </div>
                            <div>
                                <label for="fechaInicio">*Fecha de inicio (aaaa-mm-dd)</label>
                                <input type="text" class="w8em format-y-m-d divider-dash highlight-days-67" id="fechaInicio"   name="fechaInicio" value="<%=garantia.get(8)%>" size="10" MAXLENGTH="10">
                            </div>
                            <div>
                                <label for="fechaFin">*Fecha de termino (aaaa-mm-dd)</label>
                                <input type="text" class="w8em format-y-m-d divider-dash highlight-days-67" id="fechaFin"   name="fechaFin" value="<%=garantia.get(9)%>" size="10" MAXLENGTH="10">
                            </div>
                            <div id="botonEnviarDiv" >
                                <input type="button" value="Guardar" name="Enviar" onclick="enviarInfo(document.getElementById('updateGarantia'));"/>
                            </div>      
                        </fieldset>
                    </form>
                    <div id="divResult"> 
