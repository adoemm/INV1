    ...
    try {
        if (fine) {
            if (request.getParameter(WebUtil.encode(session, "imix")) != null) {
                ...
                access4ThisPage.add("ConsultaPBD");
...
<html lang="<%=PageParameters.getParameter("Content-Language")%>">
    <head>
        <title>Proveedores del Bien</title>
    ...
    <body  onload="populate();">
                ...
                <div class="form-container" width="100%">
                    ...
                    <div id="contenent_info" >
                        <form name="usuario" method="post" action="" enctype="application/x-www-form-urlencoded" id="form">
                            <fieldset>
                                <legend>Proveedores del Bien</legend>
                    ...
                    <a href="<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("gui")%>/InsertPBT.jsp?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&idBien=<%=request.getParameter("idBien")%>&idPlantel=<%=request.getParameter("idPlantel")%>">
                        <button>Asignar Proveedor</button>
                    </a> 
                </div>
                <div id="divFoot">
        ...
        function populate()
        {
            ...
            var cells = new Array();
                    ...
                    <%
                        int cont = 0;
                        it = QUID.select_PBT(WebUtil.decode(session, request.getParameter("idBien"))).iterator();
                        while (it.hasNext()) {
            ...
            cells[4] = '<a href="<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("gui")%>/UpdateProveedor.jsp?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&idProveedor=<%=WebUtil.encode(session, listAux.get(0))%>" target="_blank"><img src="<%=PageParameters.getParameter("imgRsc")%>/icons/Gnome-Accessories-Text-Editor-64.png" title="Detalles" width="22" height="23" alt="Detalles"></a>';
            cells[5] = '<a href="<%=PageParameters.getParameter("mainController")%>?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&idPBT=<%=WebUtil.encode(session, listAux.get(7))%>&idBien=<%=request.getParameter("idBien")%>&FormFrom=deletePBT&idPlantel=<%=request.getParameter("idPlantel")%>"><img src="<%=PageParameters.getParameter("imgRsc")%>/icons/Gnome-Process-Stop-64.png" title="Eliminar del Bien" width="22" height="23" alt="Eliminar del Bien"></a>';
