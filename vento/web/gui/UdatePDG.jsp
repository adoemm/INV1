...
<html lang="<%=PageParameters.getParameter("Content-Language")%>">
    <head>
        <title>Mover Agrupaciónes</title>
            ...
            function getGrupo(idPlantel) {
                if (idPlantel !== null) {
                    $.ajax({type: 'POST'
                        ...
                        , data: '<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&idPlantel=' + idPlantel+'&updatePDG=<%=WebUtil.encode(session,"1")%>'
                        , success: function(response) {
                            $('#wrapper').find('#divGrupos').html(response);
                        }});
                }
    ...
    <body onload="getDepartamento(document.getElementById('idPlantel').value);
            getGrupo('<%=request.getParameter("idPlantel")%>');">
        <div id="wrapper">
            <div id="divBody">
                ...
                <div class="form-container" width="100%">                    
                    ...
                    <form name="updatePDG" method="post" action="" enctype="application/x-www-form-urlencoded" id="updatePDG">
                        <input type="hidden" name="<%=WebUtil.encode(session, "imix")%>" value="<%=WebUtil.encode(session, UTime.getTimeMilis())%>"/>
                        <input type="hidden" name="FormFrom" value="updatePDG"/>
                        <fieldset>
                            <legend>Mover Agrupación</legend>
                            <div id="divGrupos"  align="center" style="overflow: scroll;max-height: 350px;"></div>
                            ...
                            <div id="botonEnviarDiv" >
                                <input type="button" value="Guardar" name="Enviar" onclick="enviarInfo(document.getElementById('updatePDG'));"/>
                            </div>      
                        </fieldset>
                    </form>
                    <div id="divResult"> 
