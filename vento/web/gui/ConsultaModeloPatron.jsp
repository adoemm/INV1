...
<%    try {
        if (fine) {
            if (request.getParameter(WebUtil.encode(session, "imix")) != null) {
                ...
                access4ThisPage.add("ConsultaModeloPatron");
...
<html lang="<%=PageParameters.getParameter("Content-Language")%>">
    <head>
        <title>Patrones del Modelo</title>
            ...
            function habilitarEditarColumna() {
                ...
                oTable.$("td[id!='rOnly']").editable('<%=PageParameters.getParameter("mainController")%>', {
                    ...
                    , "submitdata": function (value, settings) {
                        $("#res").val("ID_Patron_Serie" + this.parentNode.getAttribute('id'));
                        return {
                            _method: "post"
                            , "row_id": this.parentNode.getAttribute('id')
                            , "column": oTable.fnGetPosition(this)[1]
                            , "FormFrom": 'actualizarPatronSerie'
                ...
                <div class="form-container" width="100%">
                    ...
                    <div id="contenent_info" style="display:none;">
                        <form name="usuario" method="post" action="" enctype="application/x-www-form-urlencoded" id="form">
                            <fieldset>
                                <legend>Patrones del Modelo</legend>
                                <%
                                LinkedList datosModelo = QUID.select_Modelo(WebUtil.decode(session, request.getParameter("ID_Modelo")));
                                %>
                                <p align="left">Modelo Consultado: <%=datosModelo.get(2) + " " + datosModelo.get(5)%></p>
                                ...
                                <table border="0" width="100%" cellpadding="0" cellspacing="0" class="display" id="example">
                                    ...
                                    <tbody>
                                        <%
                                            int i = 1;
                                            it = QUID.select_Patron_Serie(WebUtil.decode(session, request.getParameter("ID_Modelo"))).iterator();
                                            while (it.hasNext()) {
                                        ...
                                        <tr id="<%=WebUtil.encode(session, listAux.get(0))%>" class="gradez">       
                                            ...
                                            <td id="rOnly"><%=listAux.get(7)%></td>
                                            <td id="rOnly" class="rOnly"><a href="<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("excelReports")%>/ConsultaXPatron.jsp?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&idPlantel=<%=WebUtil.encode(session,"Todos")%>&patron=<%=listAux.get(1)%>&match=sm" target="_blank"><img src="<%=PageParameters.getParameter("imgRsc")%>/icons/Gnome-Zoom-Fit-Best-64.png" title="Buscar en sin modelo en todos los planteles" width="22" height="23" alt="Buscar en sin modelo en todos los planteles"></a></td>
                                            <td id="rOnly" class="rOnly"><a href="<%=PageParameters.getParameter("mainController")%>?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&ID_Patron_Serie=<%=WebUtil.encode(session, listAux.get(0))%>&FormFrom=deletePatronSerie&ID_Modelo=<%=request.getParameter("ID_Modelo")%>"><img src="<%=PageParameters.getParameter("imgRsc")%>/icons/Gnome-Process-Stop-64.png" title="Eliminar" width="22" height="23" alt="Eliminar"></a></td>
                                        </tr>
                                        <%
                                                i += 1;
                                            }
