        ...
        <script type="text/javascript" language="javascript" charset="utf-8">
            ...
            function enviarInfo(form, action) {
                if (action === "imprimir") {
                    ...
                    form.action = '<%=PageParameters.getParameter("mainContext") + PageParameters.getParameter("excelReports")%>/Totales4Subcategoria.jsp?<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>';
                            }
                            form.submit();
                        }
                        function getSubcategoria(idCategoria) {
                            if (idCategoria !== null) {
                                $.ajax({type: 'POST'
                                    ...
                                    , data: '<%=WebUtil.encode(session, "imix")%>=<%=WebUtil.encode(session, UTime.getTimeMilis())%>&idCategoria=' + idCategoria + '&inCheck=<%=WebUtil.encode(session, "si")%>'
